  ...
  return (
    <div className="dd-menu-container">
      <button className="dd-menu-trigger" onClick={onClick}>
        ...
        >{`${currentUser}@school.org`}</span>
        <VectorDisplay
          vector={"arrow"}
          uiState={`${isActive ? "open" : "closed"}`}
        />
      </button>

      <nav
        ref={dropDownRef}
        className={`dd-menu ${isActive ? "active" : "inactive"}`}
      >
        <ul>
          <li
            className={"dd-menu-header"}
            onClick={() => setAccount(!teacherAccount)}
          >
            <div className="button">{`${switchUser}@school.org`}</div>
          </li>
          {menuContent.map((item) => {
            return (
              <li className={"dd-menu-li"} key={item.id}>
                <a className="dd-menu-item" href={item.url}>
                  {" "}
                  {item.title}
                </a>
              </li>
